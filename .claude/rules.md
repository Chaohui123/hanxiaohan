...
1. 所有 Agent 模块在做出业务决策前，必须先查询对应 RAG 知识库获取历史经验和参考信息
2. 知识库查询失败时允许降级继续执行，但必须在日志中记录降级事件
3. 新增业务模块时，必须同步实现 RAG 知识库的写入和查询逻辑
4. 详见 `docs/rag-mandatory-reference.md` 获取完整的强制引用规则和 API 端点清单