...
## 九、当前工作状态（交接快照 2026-07-30，供新会话衔接）

### 在售商品（2 个）
| offer_id | product_id | 商品 | 价格 | 状态 |
|---|---|---|---|---|
| HS-XP2-MAG-01 | 5683403180 | 车载磁吸支架（吸盘） | 99 CNY（~1146₽） | 在售，评级 100 高，评价积分中，**唯一有毛利投放空间** |
| HS-MOTO-6CLAW-01 | 5720833147 | 六爪气囊防震手机支架（摩托） | 52 CNY（~599₽） | 在售，评级回升中，零投放空间只走自然流 |

### 广告（双层流，已启动）
- **CPC 已启动**：campaign **33706085**，磁吸支架，搜索与推荐位，**2000₹/周，后付费**（结算款扣，无需预充）。监测：CTR≥1.5%、ROAS≥11.5 保本线，不达标即停。
- **CPS**：新品期不可用（全店 22% 锁定已关闭，单品需 14 天 1 单）——出单 14 天后再开单品档。
- **Yandex 站外**：用户未充值，**暂停**。启动条件：Yandex Direct 充值到账 + Ozon Performance API key（Client ID `98932307-...@advertising.performance.ozon.ru` 已在手，**Secret 已在聊天记录暴露，建议用户重置**）配到 Yandex「Мастер кампаний → Продажи на маркетплейсах」，ДРР ≤8.4% 止损。
- **双层流结论**：70% Yandex 站外 + 30% Ozon 站内按订单收费 → 实测修正为**站内 CPC 集中打支架 + CPS 暂不可 + Yandex 待充值**。火力只打磁吸支架一个 SKU。

### 关键工具（全部固化在 scripts/）
- `download-1688-assets.cjs`：1688 素材抓取（100% 成功率）
- `filter-images.py`：素材分辨率审计（HD≥800 主图）
- `upload-frames.mjs`：传图到服务器（/api/image/upload）
- `make-video-ru.py`：中文视频俄化（去音轨+俄文字幕）
- `upload-ozon-video.cjs`：WebBridge 卖家后台传视频
- `validate-rich-json.cjs` + `assets/ozon-rich-schema.json`：富内容本地预检
- `temp/ozon-api.cjs`：Ozon Seller API 客户端（读 .env OZON_CLIENT_IDS/API_KEYS）
- `temp/perf-api.cjs`：Ozon Performance API（广告）客户端（token+campaign）

### Agent 架构（SOP v1.2，`docs/agent-sop.md`）
总控 + 6 专家（选品/视觉/文案/定价/质检/广告）。流水线：选品 GO → 视觉∥文案∥定价并行 → 总控上架 → 质检 PASS → 广告。踩坑库 `docs/pitfalls.md`（30+ 条，子 Agent 启动前必读）。

### 下一步待办（按优先级）
1. **CPC 数据监测**（3-5 天后）：campaign 33706085 的 CTR/ROAS，不达标调整或停
2. **摩托支架评级复验**（补图/视频/属性后的重算值）
3. **双层流 BASE 引流款**（定价 Agent 建议：48.9¥ 基础款 1099-1199₽ 走量 + 62.9¥ 数显款 1439₽ 利润锚点）——搅拌杯 1688 同链接两变体，素材基本复用
4. **测款池扩展**：继续选品（跨境专供/白底图多标准 3C 配件优先，HD 干净图≥3 才立项）
5. **Yandex 充值后**：启动站外曝光（ДРР≤8.4%）
6. **买家问答区**：每日看"买家→Вопросы-ответы"回复（跨境店主要买家沟通渠道，聊天功能灰度未开放）

### 用户偏好（必须遵守）
- 子 Agent 思考链路**统一中文**（便于监督纠正）
- 流程：选完交子 Agent 优化建议 → 优化后复核 → 总控上架 → 质检验收
- **禁裁剪中文供应商图**；素材不够走"卖点提炼俄文重制"（PIL 模板，但**同模板信息图 ≤2 张防雷同判重**）
- 评级重算 ~5 分钟非每日；图集补图后等 ≥10 分钟管线稳定再判真实图数（瞬时 0 ≠ 缺失）
