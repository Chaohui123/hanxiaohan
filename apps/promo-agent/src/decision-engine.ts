...
export interface ProductScore {
  ...
  priceAdvantage: number; // 正值=比竞品便宜
...
export function scorePriceAdvantage(advantagePct: number): number {
  ...
  return (capped + 30) / 60; // 映射 [-30, 30] → [0, 1]
}
...
export function getRecommendation(
  ...
  if (needCopy && needPricing) {
    return { recommendation: "copy_and_pricing", reason: `利润高但转化低 + 价格需优化` };
  }
  if (needCopy) {
    return { recommendation: "copy", reason: `利润率 ${marginPct.toFixed(0)}% 良好，优化文案提升转化` };
  }
  if (needPricing) {
    return { recommendation: "pricing", reason: `价格竞争力不足 (优势${priceAdvantagePct.toFixed(0)}%)，需调价` };
  }
