...
export async function aiDiagnose(config: ApiConfig, rawData: string): Promise<string> {
  ...
  // RAG 知识库查询
  const diagResults = await queryRag(config, "playbook", `诊断 ${rawData.slice(0, 100)}`, { scenario: "diagnosis", topK: 5 });
  const ragContext = diagResults.length
    ? `以下是历史诊断经验参考：\n${diagResults.map((r: { content: string }) => r.content).join("\n")}\n\n请结合以上经验进行诊断。`
    : "";
  ...
  try {
    ...
    writeRag(config, "playbook", {
      title: `AI 诊断: ${rawData.slice(0, 50)}`, scenario: "diagnosis",
      content: `诊断结果:\n${content.slice(0, 500)}`,
      tags: ["诊断", "AI"], author: "ai-diagnose", priority: 1,
    });
